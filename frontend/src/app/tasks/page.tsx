...
import { fetchTasks, createTask, updateTask, deleteTask, completeTask, fetchUser } from '@/utils/api';
...
export default function Tasks() {
    ...
    const handleCreateTask = (newTask:{title:string, description:string, priority:string})=>{
        createTaskMutation.mutate(newTask);
        handleAddTaskDialogClose();
    }
    const handleTaskSubmit = (taskData: TaskFormData | (TaskFormData & { id: string })) => {
      if ('id' in taskData) {
    ...
    <div className="bg-zinc-950 h-screen">
        ...
        {sideBarOpen && <Sidebar open={sideBarOpen} />}
        <div className="flex flex-grow justify-around">
        <Button className='my-4 bg-zinc-700 hover:bg-lime-900' onClick={handleAddTaksButtonClick}>Add New Task</Button>
            <TaskForm 
