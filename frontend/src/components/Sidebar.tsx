...
export default function Sidebar({open,userLoggedIn,userName,userImage}:Sidebarprops) {
    ...
    return (
        <div className={clsx('absolute z-10 top-0 left-0 bg-zinc-800 w-[175px] h-screen transition-transform duration-300',{'transform -translate-x-full':!open, 'transform translate-x-0':open})}>
            ...
            {userLoggedIn? (
                < Link className='text-lime-600 font-medium w-full text-center my-1 px-12 py-2 hover:bg-zinc-700 hover:text-lime-400 transition-all duration-300 ease-in-out' href='/auth/login'>Login</Link>
            ):
            (
                <Link className='text-lime-600 font-medium w-full text-center my-1 px-12 py-2 hover:bg-zinc-700 hover:text-lime-400 transition-all duration-300 ease-in-out' href='/auth/logout'>Logout</Link>
            )}
            </div>
        </div>
    );
